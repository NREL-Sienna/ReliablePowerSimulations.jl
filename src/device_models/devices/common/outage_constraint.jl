function device_outage_parameter!(
    ...
    con_on = PSI.add_constraints_container!(container, T(), V, device_names, time_steps)
    ...
    for ic in initial_conditions_outage
        ...
        con_on[name, 1] =
            JuMP.@constraint(container.JuMPmodel, varon[name, 1] <= param[name, 1])
        con_start[name, 1] =
            JuMP.@constraint(container.JuMPmodel, varstart[name, 1] <= param[name, 1])
        ...
        for t in time_steps[2:end]
            con_on[name, t] =
                JuMP.@constraint(container.JuMPmodel, varon[name, t] <= param[name, t])
            con_start[name, t] =
                JuMP.@constraint(container.JuMPmodel, varstart[name, t] <= param[name, t])
            con_stop[name, t] = JuMP.@constraint(
                container.JuMPmodel,
                varstop[name, t] >=
                param[name, t - 1] - param[name, t] - (1.0 - varon[name, t])
            )
        end
    end
    return
...
function device_outage_ub_parameter!(
    ...
    constraint = PSI.add_constraints_container!(container, T(), V, device_names, time_steps)
    param = PSI.get_parameter_array(container, OutageTimeSeriesParameter(), V)
    multiplier =
        PSI.get_parameter_multiplier_array(container, OutageTimeSeriesParameter(), V)
