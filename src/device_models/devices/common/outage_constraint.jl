function device_outage_parameter!(
    ...
    initial_conditions_outage = PSI.get_initial_condition(container, InitialOutageStatus(), U)
...
function device_outage_ub_parameter!(
    ...
    V <: Union{PSY.HydroGen, PSY.HydroPumpedStorage, PSY.RenewableGen, PSY.Storage, PSY.ThermalGen}, 
    S <: PSI.VariableType,
}
    time_steps = PSI.get_time_steps(container)
    device_names = [PSY.get_name(d) for d in devices]
